...
#Google Data#
#############
google_raw<-read.csv("C:/Users/dougl/Desktop/comp-syn/algorithmic_bias/Data/RnR/Google_experiment_full_final_pub.csv")
...
##############################
wiki_raw<-read.csv("C:/Users/dougl/Desktop/comp-syn/algorithmic_bias/Data/RnR/Wiki_experiment_full_final_pub.csv")
...
#############################
google_demo_simp<-unique(google_clean %>% select(source, WorkerId, mt_poli.orient, mt_gender, mt_race, mt_age, mt_income, mt_education))
wiki_demo_simp<-unique(wiki_clean %>% select(source, WorkerId, mt_poli.orient, mt_gender, mt_race, mt_age, mt_income, mt_education))
...
google_clean_simp_wNumFaces<-google_clean_simp %>% group_by(image_id) %>% 
  dplyr::summarise(num_faces = length(unique(face_id)))

google_clean_simp_wNumFaces_full<-merge(google_clean_simp, google_clean_simp_wNumFaces, by=c("image_id"))

Google_mod_demo_wNumFace<-lm(img.age ~ img.gender + num_faces, data = google_clean_simp_wNumFaces_full)
summary(Google_mod_demo_wNumFace)

Google_mod_demo_wNumFace_vcov <- cluster.vcov(Google_mod_demo_wNumFace, google_clean_simp_wNumFaces_full$Social.Category)
coeftest(Google_mod_demo_wNumFace, Google_mod_demo_wNumFace_vcov)

#Controlling for frequency in Google Trends 
google_trends_freq<-read.csv("C:/Users/dougl/Desktop/comp-syn/algorithmic_bias/Data/RnR/category_Google_frequency.csv")
...
t.test(subset(google_clean_agg, img.gender.mode == "Female")$img.age.avg, 
       subset(google_clean_agg, img.gender.mode == "Male")$img.age.avg)
...
uncropped<-read.csv("C:/Users/dougl/Desktop/comp-syn/algorithmic_bias/Data/RnR/Google_replication_uncropped_pub.csv")
